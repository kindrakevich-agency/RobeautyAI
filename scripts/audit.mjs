/**
 * Аудит інтерфейсу: обидві теми × усі маршрути.
 *
 * Шукає те, що око пропускає: горизонтальний скрол, текст, який зливається
 * з фактичним тлом, порожні розділи, зниклий сайдбар, помилки консолі й
 * запити зі статусом 4xx/5xx.
 *
...
for (const theme of ['light', 'dark']) {
  ...
  for (const route of ROUTES) {
    ...
    const issues = []
    if (res.overflow > 2) issues.push(`горизонтальний скрол ${res.overflow}px`)
    ...
    if (issues.length) bugs.push({ theme, route, issues })
...
else for (const x of bugs) console.log(`✗ [${x.theme}] ${x.route}\n    ${x.issues.join('\n    ')}`)
